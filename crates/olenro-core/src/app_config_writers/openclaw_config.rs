...
//!
//! 本模块由桌面端 `src-tauri/src/openclaw_config.rs` 移植而来，逻辑保持一致，
//! 仅将错误类型适配到 `olenro_core::error::AppError`，并去掉桌面端 settings
//! 覆盖目录依赖（CLI 固定使用 `~/.openclaw`）。
...
/// 保留的 openclaw 配置备份数量（桌面端可配置，CLI 固定）。
const OPENCLAW_BACKUP_RETAIN_COUNT: usize = 10;
...
impl OpenClawConfigDocument {
    ...
    fn save(self) -> Result<OpenClawWriteOutcome, AppError> {
        let _guard = openclaw_write_lock()
            .lock()
            .map_err(|_| AppError::Config("OpenClaw config write lock poisoned".to_string()))?;
...
fn cleanup_openclaw_backups(dir: &Path) -> Result<(), AppError> {
    let retain = OPENCLAW_BACKUP_RETAIN_COUNT;
...
mod tests {
    ...
    fn with_test_paths<T>(source: &str, test: impl FnOnce(&Path) -> T) -> T {
        ...
        let old_test_home = std::env::var_os("OLENRO_TEST_HOME");
        std::env::set_var("OLENRO_TEST_HOME", temp.path());
