...
/// 统一处理 olenro:// 深链接 URL（兼容旧版 ccswitch://）
///
/// - 解析 URL
...
fn handle_deeplink_url(
    ...
    if !url_str.starts_with("olenro://") && !url_str.starts_with("ccswitch://") {
        return false;
    }
...
pub fn run() {
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            crate::config::migrate_legacy_config_dir_if_needed();
            ...
            // 初始化日志（单文件输出到 <app_config_dir>/logs/olenro.log）
            {
                ...
                let log_file_path = log_dir.join("olenro.log");
            ...
            let db_path = app_config_dir.join(crate::config::DB_FILENAME);
            ...
            #[cfg(any(target_os = "linux", all(debug_assertions, windows)))]
            {
                #[cfg(target_os = "linux")]
                {
                    ...
                    // tauri-plugin-deep-link writes to: ~/.local/share/com.olenro.desktop/applications/olenro-handler.desktop
                    // Only register if .desktop file doesn't exist to avoid overwriting user customizations
                    let should_register = app
                        .path()
                        .data_dir()
                        .map(|d| !d.join("applications/olenro-handler.desktop").exists())
                        .unwrap_or(true);
                ...
                move |event| {
                    ...
                    for (i, url) in urls.iter().enumerate() {
                        ...
                        if handle_deeplink_url(&app_handle, url_str, true, "on_open_url") {
                            break; // Process only first olenro:// (or legacy ccswitch://) URL
                        }
                    }
                }
            });
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::git_sync_save_settings,
            commands::git_test_connection,
            commands::git_sync_push,
            commands::git_sync_force_push,
            commands::git_sync_pull,
            commands::git_sync_fetch_remote_info,
            commands::git_sync_asset_catalog,
            ...
            commands::get_popular_skills_sh,
    ...
    app.run(|app_handle, event| {
        ...
        #[cfg(target_os = "macos")]
        {
            match event {
                ...
                // 处理通过自定义 URL 协议触发的打开事件（例如 olenro://... 或旧版 ccswitch://...）
...
fn show_database_init_error_dialog(
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Suggestions:\n\
            1) Back up the entire config directory (including olenro.db)\n\
