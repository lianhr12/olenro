...
//!
//! Parses olenro:// (and legacy ccswitch://) URLs into DeepLinkImportRequest structures.
...
/// Parse an olenro:// (or legacy ccswitch://) URL into a DeepLinkImportRequest
///
/// Expected format:
/// olenro://v1/import?resource={type}&...
pub fn parse_deeplink_url(url_str: &str) -> Result<DeepLinkImportRequest, AppError> {
    ...
    // Validate scheme. The app registers `olenro://`; the legacy `ccswitch://`
    // scheme is still accepted so users can import existing Olenro share links.
    let scheme = url.scheme();
    if scheme != "olenro" && scheme != "ccswitch" {
        return Err(AppError::InvalidInput(format!(
            "Invalid scheme: expected 'olenro' (or legacy 'ccswitch'), got '{scheme}'"
        )));
    }
