...
const CC_SWITCH_SQL_EXPORT_HEADER: &str = "-- Olenro SQLite 导出";
...
impl Database {
    ...
    fn validate_cc_switch_sql_export(sql: &str) -> Result<(), AppError> {
        ...
        Err(AppError::localized(
            ...
            "仅支持导入由 Olenro 导出的 SQL 备份文件。",
            "Only SQL backups exported by Olenro are supported.",
        ))
    }
    ...
    pub(crate) fn backup_database_file(&self) -> Result<Option<PathBuf>, AppError> {
        let db_path = get_app_config_dir().join(crate::config::DB_FILENAME);
        if !db_path.exists() {
            return Ok(None);
        }
