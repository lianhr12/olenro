...
impl SkillService {
    ...
    /// 获取 SSOT 目录（根据设置返回 ~/.olenro/skills/ 或 ~/.agents/skills/）
    ...
    /// 获取 Skill 卸载备份目录（~/.olenro/skill-backups/）
    ...
    pub async fn search_skills_sh(
        ...
        let skills = resp
            ...
            .into_iter()
            .filter_map(|s| {
                Self::map_skillssh_skill(&s.source, s.id, s.skill_id, s.name, s.installs)
            })
            .collect();
    ...
    /// 获取 skills.sh 的热门技能（按安装量倒序）
    ///
    /// skills.sh 没有公开的“热门”接口（/api/search 要求查询至少 2 个字符，
    /// /api/v1/skills 需要 API key），但首页将按安装量排序的技能列表内联在
    /// Next.js 的 flight 数据中。这里抓取首页 HTML 并解析出这些条目，
    /// 用于在用户尚未输入搜索词时默认展示热门技能。
    pub async fn get_popular_skills_sh(limit: usize) -> Result<SkillsShSearchResult> {
        let client = crate::proxy::http_client::get();
        ...
        // 首页 flight 数据中的技能条目（转义后的 JSON），形如：
        // \"source\":\"owner/repo\",\"skillId\":\"...\",\"name\":\"...\",\"installs\":123
        ...
        for caps in re.captures_iter(&html) {
            ...
            // 同一技能在首页可能出现多次（如多个榜单），按 source/skillId 去重
            if !seen.insert(format!("{source}/{skill_id}")) {
                continue;
            }
            let key = format!("{source}/{skill_id}");
            if let Some(skill) = Self::map_skillssh_skill(source, key, skill_id, name, installs) {
                skills.push(skill);
            }
            if skills.len() >= limit {
                break;
    ...
    /// 将 skills.sh 的原始技能字段映射为前端可用的可安装技能。
    ///
    /// `source` 形如 `owner/repo`，会被拆分为仓库 owner/name；非 GitHub 来源
    /// （如 `skills.volces.com`、`mcp-hub.momenta.works`）会被过滤掉。
    fn map_skillssh_skill(
        source: &str,
        key: String,
        skill_id: String,
        name: String,
