...
/// Git 同步设置
///
/// 取代 WebDAV 同步：把资产以明文目录形式提交到一个私有 Git 仓库，
/// 通过 HTTPS + PAT 鉴权，手动推/拉。
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GitSyncSettings {
    #[serde(default)]
    pub enabled: bool,
    /// 远程仓库 HTTPS 地址，例如 https://github.com/owner/repo.git
    #[serde(default)]
    pub repo_url: String,
    /// 分支名，默认 main
    #[serde(default = "default_git_branch")]
    pub branch: String,
    /// HTTPS 用户名（GitHub 可填任意非空值，例如 "x-access-token" 或用户名）
    #[serde(default)]
    pub username: String,
    /// Personal Access Token（明文存储于本地 settings.json，不随仓库同步）
    #[serde(default)]
    pub token: String,
    /// 提交作者名
    #[serde(default)]
    ...
    /// 本设备名（写入 manifest，便于多设备识别）
    #[serde(default)]
    pub device_name: String,
    /// 已关闭同步的资产 ID 列表（默认空 = 全部启用）。
    /// 仅作用于「agent 资产」（settings/hooks、subagents、commands、记忆文件）；
    /// 供应商 / MCP / Skills 始终同步。
    #[serde(default)]
...
impl GitSyncSettings {
    pub fn validate(&self) -> Result<(), crate::error::AppError> {
        ...
        if !(url.starts_with("https://") || url.starts_with("http://")) {
            return Err(crate::error::AppError::localized(
                "git.repo_url.https_only",
                "目前仅支持 HTTPS 仓库地址（https://...）",
                "Only HTTPS repository URLs are supported (https://...).",
            ));
        }
        Ok(())
    }
    ...
    pub fn normalize(&mut self) {
        ...
        if self.branch.is_empty() {
            self.branch = default_git_branch();
        }
        if self.username.is_empty() {
            self.username = "x-access-token".to_string();
        }
        if self.author_name.is_empty() {
            self.author_name = "olenro".to_string();
        }
        if self.author_email.is_empty() {
            self.author_email = "olenro@local".to_string();
        }
    }
}
...
/// 应用设置结构
///
/// 存储设备级别设置，保存在本地 `~/.olenro/settings.json`，不随数据库同步。
/// 这确保了云同步场景下多设备可以独立运作。
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct AppSettings {
    ...
    // ===== Git 同步设置（取代 WebDAV） =====
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub git_sync: Option<GitSyncSettings>,
...
impl AppSettings {
    ...
    fn normalize_paths(&mut self) {
        ...
        if let Some(sync) = &mut self.git_sync {
            sync.normalize();
            if sync.repo_url.is_empty() && sync.token.is_empty() {
                self.git_sync = None;
            }
        }
    }
...
pub fn get_settings_for_frontend() -> AppSettings {
    ...
    // 不把 Git PAT 暴露给前端；前端保存时空 token 表示「保持现有」。
    if let Some(sync) = &mut settings.git_sync {
        sync.token.clear();
    }
    settings
}
...
/// 获取 Git 同步设置
pub fn get_git_sync_settings() -> Option<GitSyncSettings> {
    settings_store().read().ok()?.git_sync.clone()
}

/// 保存 Git 同步设置
pub fn set_git_sync_settings(settings: Option<GitSyncSettings>) -> Result<(), AppError> {
    mutate_settings(|current| {
        current.git_sync = settings;
    })
}

/// 仅更新 Git 同步状态，避免覆写 credentials/repo 等字段
pub fn update_git_sync_status(status: GitSyncStatus) -> Result<(), AppError> {
    mutate_settings(|current| {
        if let Some(sync) = current.git_sync.as_mut() {
            sync.status = status;
        }
    })
}
