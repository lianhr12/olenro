...
"#
    ...
    // Current design: third-party providers are written to live config.toml as-is
    // (prepare_codex_provider_live_config only injects the bearer token), so the
    // provider's real model_provider id is preserved — matching this test's name.
    assert_eq!(
        parsed.get("model_provider").and_then(|v| v.as_str()),
        Some("aihubmix"),
        "live sync should preserve the provider's real model_provider id"
    );
    ...
    assert_eq!(
        ...
        "live config should keep the provider-specific model_providers table"
    );
...
fn create_backup_skips_missing_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_path = home.join(".olenro").join("config.json");
...
fn create_backup_generates_snapshot_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_dir = home.join(".olenro");
...
fn sync_gemini_packycode_sets_security_selected_type() {
    ...
    // security field is written to ~/.gemini/settings.json, not ~/.olenro/settings.json
...
fn export_sql_returns_error_for_invalid_path() {
    ...
    let invalid_parent = if cfg!(windows) {
        std::env::temp_dir().join("olenro-test-invalid<>dir")
    } else {
...
fn import_sql_accepts_cc_switch_exported_backup() {
    ...
    let state = create_test_state_with_config(&config).expect("create test state");
    let export_path = home.join("olenro-export.sql");
    state
        .db
        .export_sql(&export_path)
        .expect("export should succeed");
