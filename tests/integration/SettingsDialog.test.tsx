  ...
  return render(
    <QueryClientProvider client={client}>
      <Suspense fallback={<div data-testid="loading">loading</div>}>
        <SettingsPage open {...props} />
      </Suspense>
    </QueryClientProvider>,
  );
};

// 新版设置为九宫格入口；空 i18n 资源下卡片标题即为 i18n key
const openDirectories = async () =>
  fireEvent.click(await screen.findByText("settingsGrid.directories"));
const openData = async () =>
  fireEvent.click(await screen.findByText("settingsGrid.data"));
...
describe("SettingsPage integration", () => {
  it("loads default settings from MSW", async () => {
    ...
    expect((appInput as HTMLInputElement).value).toBe("/home/mock/.olenro");
  });
  ...
  it("allows browsing and resetting directories", async () => {
    ...
    expect(appInput.value).toBe("/home/mock/.olenro");

    fireEvent.click(browseButtons[0]);
    await waitFor(() =>
      expect(appInput.value).toBe("/home/mock/.olenro/picked"),
    );

    fireEvent.click(resetButtons[0]);
    await waitFor(() => expect(appInput.value).toBe("/home/mock/.olenro"));
