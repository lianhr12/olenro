...
// Sidebar 现在承载 App 选择器与功能导航；以轻量 mock 暴露切换/导航按钮
vi.mock("@/components/shell/Sidebar", () => ({
  Sidebar: ({ activeApp, onSwitchApp, onNavigate }: any) => (
    <div data-testid="sidebar">
      <span>{activeApp}</span>
      <button onClick={() => onSwitchApp("claude")}>switch-claude</button>
      <button onClick={() => onSwitchApp("codex")}>switch-codex</button>
      <button onClick={() => onSwitchApp("openclaw")}>switch-openclaw</button>
      <button onClick={() => onNavigate("providers")}>nav-providers</button>
...
describe("App integration with MSW", () => {
  beforeEach(() => {
    ...
    // 默认落地页现为 dashboard；测试聚焦 provider 流程，强制初始视图为 providers
    localStorage.setItem("olenro-last-view", "providers");
  });
