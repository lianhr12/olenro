...
vi.mock("@/components/settings/GitSyncSection", () => ({
  GitSyncSection: ({ config }: any) => (
    <div>git-sync-section:{config?.repoUrl ?? "none"}</div>
  ),
}));
...
const renderSettingsPage = (
  ...
  return render(
    <QueryClientProvider client={client}>
      <SettingsPage open={true} {...props} />
    </QueryClientProvider>,
  );
};

// 新版设置为九宫格入口 + 二级详情：点击分类卡片进入对应详情
const openCategory = (titleKey: string) =>
...
describe("SettingsPage Component", () => {
  ...
  it("should render category grid and drill into general / appearance / data / cloudSync", () => {
    ...
    // 入口网格：分类卡片标题为 i18n key（tMock 直返 key）
    expect(screen.getByText("settingsGrid.general")).toBeInTheDocument();
    expect(screen.getByText("settingsGrid.cloudSync")).toBeInTheDocument();

    // 通用详情
    openCategory("settingsGrid.general");
    ...
    // 外观详情：主题
    openCategory("settingsGrid.appearance");
    expect(screen.getByText("theme-settings")).toBeInTheDocument();
    goBackToGrid();

    // 云同步详情
    openCategory("settingsGrid.cloudSync");
    expect(screen.getByText("git-sync-section:none")).toBeInTheDocument();
    goBackToGrid();

    // 数据与备份详情：导入 / 导出 / 清除（来自真实 ImportExportSection）
    openCategory("settingsGrid.data");
  ...
  it("should call saveSettings and return to grid when clicking save", async () => {
    importExportMock = createImportExportMock();

    renderSettingsPage();

    // 保存按钮位于目录(directories)详情底部
    openCategory("settingsGrid.directories");
    ...
    // 保存成功后返回入口网格
    expect(screen.getByText("settingsGrid.directories")).toBeInTheDocument();
  });
  ...
  it("should trigger directory management callbacks inside directories detail", () => {
    renderSettingsPage();
