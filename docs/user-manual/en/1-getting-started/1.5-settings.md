...
This section describes how to configure Olenro according to your preferences.
...
Olenro supports three languages:
...
When enabled, Olenro automatically runs when the system starts.
...
Starting from v3.13.0, Olenro adds **Lightweight Mode** — a **tray-only** running state that minimizes desktop footprint when idle.
...
**How to exit**: Click **Open Main Window** from the tray menu, or trigger Olenro via deep link / relaunch. The window is **rebuilt on demand**, with state preserved.
...
> **Use case**: If Olenro runs in the background for long periods and you mainly switch providers via the tray menu, enabling Lightweight Mode significantly reduces memory usage.
...
When enabled, Olenro automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Choose the terminal application that Olenro uses when opening a terminal.
...
The storage location for Olenro's own data, defaulting to `~/.olenro/`.
...
Configure Olenro's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, Olenro automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
| Remote Directory | Remote storage path (default: `olenro-sync`) |
...
- Olenro automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, Olenro displays details about the remote snapshot:
- Protocol version (v2)
...
Displays the current Olenro version number, with support for:
