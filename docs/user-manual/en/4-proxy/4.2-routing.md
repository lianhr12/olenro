...
App routing means letting Olenro route a specific application's API requests through the local routing service.
...
When routing is enabled, Olenro modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, Olenro will:
...
Before enabling routing, Olenro backs up the original configuration and restores it when disabled.
