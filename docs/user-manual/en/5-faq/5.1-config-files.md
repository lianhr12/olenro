...
```
~/.olenro/
├── olenro.db      # SQLite database (SSOT)
...
`olenro.db` is a SQLite database that stores:
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to Olenro's "providers") |
...
Olenro's priority when modifying configurations:

1. **Olenro Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by Olenro)
- Olenro's `settings.json`

### Not Recommended to Edit Manually

- `olenro.db` database file
- Backup files
...
Olenro v3.7.0 migrated from JSON files to SQLite:
