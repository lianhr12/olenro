...
Olenro for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the [Releases page](https://github.com/horace68/olenro/releases).
...
2. Add Olenro to your antivirus software's whitelist
...
**Solutions**:
1. Check if the `~/.olenro/` directory exists
2. Restore from backup: `~/.olenro/backups/`
...
**Solutions**:
1. Confirm the file was exported by Olenro
...
**Windows**:
- Check taskbar settings to ensure the Olenro icon is not hidden
...
2. Restart the app
3. Delete `~/.olenro/settings.json` to reset settings
...
3. If using Homebrew: `brew upgrade --cask olenro`

## Lightweight Mode

### How to Enter Lightweight Mode?

Toggle "Lightweight Mode" from the system tray menu. The main window closes, and Olenro runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. Starting from v3.13.0, Olenro covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `olenro://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
...
1. Visit [GitHub Issues](https://github.com/horace68/olenro/issues)
...
- macOS/Linux: `~/.olenro/logs/`
- Windows: `%APPDATA%\olenro\logs\`
