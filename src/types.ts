...
export interface Provider {
  ...
  // 新增：是否为商业合作伙伴
  isPartner?: boolean;
  // 可选：供应商元数据（仅存于 ~/.olenro/config.json，不写入 live 配置）
  meta?: ProviderMeta;
  // 图标配置
...
export interface GitSyncSettings {
  ...
  authorEmail?: string;
  deviceName?: string;
  // 已关闭同步的 agent 资产 ID（默认空 = 全部启用）
  disabledAssets?: string[];
  status?: GitSyncStatus;
}

// 可勾选的 agent 资产（来自后端清单）
export interface GitAssetInfo {
  id: string;
  tool: string;
  homePath: string;
  isDir: boolean;
...
// 存储在本地 ~/.olenro/settings.json，不随数据库同步
