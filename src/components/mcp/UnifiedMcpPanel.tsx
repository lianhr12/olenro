...
>(({ onOpenChange: _onOpenChange }, ref) => {
  ...
  const toggleAppsBulkMutation = useToggleMcpAppsBulk();
  ...
  const handleToggleAllApps = async (
    ...
    const targets = MCP_APP_IDS.filter(
      (app) => !!server.apps[app] !== enabled,
    ).map((app) => ({ serverId, app }));
  ...
  const globalAllEnabled = useMemo(
    () =>
      ...
      serverEntries.every(([, server]) =>
        MCP_APP_IDS.every((app) => server.apps[app]),
      ),
    [serverEntries],
  );

  const runToggleGlobal = async (enabled: boolean) => {
    const targets = serverEntries.flatMap(([id, server]) =>
      MCP_APP_IDS.filter((app) => !!server.apps[app] !== enabled).map(
        (app) => ({
          serverId: id,
          app,
        }),
  ...
  const handleToggleGlobal = () => {
    if (globalAllEnabled) {
      setConfirmDialog({
        ...
        message: t("mcp.disableAllConfirm"),
        onConfirm: async () => {
    ...
    <div className="px-6 flex flex-col flex-1 min-h-0 overflow-hidden">
      <div className="flex flex-nowrap items-center justify-between gap-3 mb-3">
        ...
        <Button
          ...
          disabled={
            serverEntries.length === 0 || toggleAppsBulkMutation.isPending
          }
        >
  ...
  return (
    <ListItemRow isLast={isLast}>
      ...
      <AppToggleGroup
        ...
        onToggleAll={(enabled) => onToggleAllApps(id, server, enabled)}
        appIds={MCP_APP_IDS}
      />
