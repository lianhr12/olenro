...
export interface GitSyncResult {
  success: boolean;
  // 推送时若检测到远端领先则为 true（需用户选择强制覆盖或先拉取）
  diverged?: boolean;
  // 本地与远端一致、无需同步
  noChanges?: boolean;
  // 拉取时远端尚无数据
...
export const settingsApi = {
  ...
  async gitSyncSaveSettings(
    settings: GitSyncSettings,
    tokenTouched = false,
  ): Promise<{ success: boolean }> {
    return await invoke("git_sync_save_settings", {
      settings,
      tokenTouched,
    });
  },

  async gitTestConnection(
    settings: GitSyncSettings,
    tokenTouched = false,
  ): Promise<{ success: boolean; message?: string }> {
    return await invoke("git_test_connection", {
      settings,
      tokenTouched,
    });
  },

  async gitSyncPush(): Promise<GitSyncResult> {
    return await invoke("git_sync_push");
  },

  async gitSyncForcePush(): Promise<GitSyncResult> {
    return await invoke("git_sync_force_push");
  },

  async gitSyncPull(): Promise<GitSyncResult> {
    return await invoke("git_sync_pull");
  },

  async gitSyncFetchRemoteInfo(): Promise<GitRemoteInfo> {
    return await invoke("git_sync_fetch_remote_info");
  },

  async gitSyncAssetCatalog(): Promise<GitAssetInfo[]> {
    return await invoke("git_sync_asset_catalog");
  },
