...
/** Skill 完整详情（包含 manifest 内容和文件列表） */
...
export const skillsApi = {
  ...
  async getPopularSkillsSh(limit: number): Promise<SkillsShSearchResult> {
    return await invoke("get_popular_skills_sh", { limit });
  },
  ...
  async getSkillDetail(skillId: string): Promise<SkillDetail> {
    return await invoke("get_skill_detail", { skillId });
  },

  /** 列出 Skill 目录下的所有文件 */
  async listSkillFiles(skillId: string): Promise<SkillFileInfo[]> {
    return await invoke("list_skill_files", { skillId });
  },
  ...
  async writeSkillFile(
    ...
    return await invoke("write_skill_file", { skillId, filePath, content });
  },

  /** 在系统文件管理器中打开 Skill 目录 */
  async openSkillDirectory(skillId: string): Promise<boolean> {
    return await invoke("open_skill_directory", { skillId });
  },
};
